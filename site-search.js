(function(){
  ...
  let root=document.querySelector('.bs-global-search-wrap');
  if(!root){
    ...
    if(nav)nav.insertAdjacentElement('afterend',root);else if(document.body)document.body.insertBefore(root,document.body.firstChild);
  }
  const css=document.createElement('style');
  css.textContent=`
  .bs-global-search-wrap{max-width:1200px;margin:10px auto;padding:0 12px;box-sizing:border-box;display:block!important;visibility:visible!important}
  .bs-global-search{position:relative;display:flex;gap:7px;max-width:680px;margin:0 auto;background:#fff;border:1px solid #d7dee7;border-radius:10px;padding:5px;box-shadow:0 2px 8px rgba(16,42,67,.06);box-sizing:border-box}
  .bs-global-search input{flex:1;min-width:0;border:0;outline:0;padding:9px 11px;font:600 14px/1.3 Arial,"Noto Sans Bengali",sans-serif;background:transparent;color:#111}
  .bs-global-search button{border:0;border-radius:7px;background:#d71920;color:#fff;font-weight:800;padding:0 15px;cursor:pointer;min-height:36px}
  ...
  @media(max-width:768px){.bs-global-search-wrap{margin:8px auto;padding:0 10px}.bs-global-search button{padding:0 12px;font-size:12px}.bs-global-search input{font-size:13px}}
  `;
  ...
  let items=[];
  const normalizeId=v=>{const s=String(v||'').trim();return /^\d+(?:\.0+)?$/.test(s)?String(parseInt(s,10)):s;};
  const escapeHtml=s=>String(s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const dataUrl=(base==='./'?'news-data.json':base+'news-data.json');
  const newsPath=id=>(base==='./'?'news/':base+'news/')+encodeURIComponent(normalizeId(id))+'.html';
  function render(){
    ...
    found.forEach(x=>{const a=document.createElement('a');a.href=new URL(newsPath(x.id),location.href).href;a.innerHTML='<strong>'+escapeHtml(x.title)+'</strong><br><small>'+escapeHtml(x.category)+'</small>';results.appendChild(a);});
    results.style.display='block';
  }
  fetch(dataUrl+'?_search_v5='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null).then(data=>{
